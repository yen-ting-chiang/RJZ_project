...
fpkm_df <- fread(file = "fpkm_with_mean.csv")
fpkm_df_filtered <- fpkm_df %>% 
  filter(mean>1)
df_filtered <- df %>% 
  filter(`Gene name` %in% fpkm_df_filtered$Gene == TRUE)

m6A_genes <- fread(file = "m6A_genes.csv")
df_m6A <- df_filtered %>% 
  filter(`Gene name` %in% m6A_genes$m6A_genes == TRUE) %>% 
  mutate(m6A_status = "m6A_genes")
df_non_m6A <- df_filtered %>% 
...
png("violin_chart_m6A_Y2_strict_fpkm_1.png",
    width = 4500,
    height = 3000,
    res = 600)
